...
bool device_breath_quit = false;
#define DEVICE_NAME "vim2"

void* DisplayMode::deviceBreath(void* data) 
{
    ...
    SYS_LOGE("device breath thread in...\n");
 

    if ((sock_fd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
        SYS_LOGE("socket create error\n");
        goto EXIT;
    }
    ...
    socket_len = (socklen_t)sizeof(struct sockaddr_in);
...
void DisplayMode::startBootanimDetectThread() {
    ...
    SYS_LOGI("Start create device breath thread...\n");
    ret = pthread_create(&id, NULL, deviceBreath, this);
    if (ret != 0) {
        SYS_LOGE("Create deviceBreath thread error!\n");
    }
}
